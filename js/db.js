...
import { OBJECT_STORES, runMigrations } from "./db-schema.js";
...
/**
 * Persists schema metadata inside the shared `meta` store in the same upgrade
 * transaction as the schema migration itself.
 *
 * @param {object} context
 * @param {import('https://cdn.jsdelivr.net/npm/idb@8.0.3/+esm').IDBPTransaction} context.transaction
 * @param {number} context.targetVersion
 */
function writeSchemaVersionInUpgradeTransaction({ transaction, targetVersion }) {
...
/**
 * Opens the application's IndexedDB database and applies schema migrations.
 *
  ...
  if (!databasePromise) {
    databasePromise = openDB(DATABASE_NAME, DATABASE_SCHEMA_VERSION, {
      upgrade(db, oldVersion, newVersion, transaction) {
        ...
        writeSchemaVersionInUpgradeTransaction({
          transaction,
          targetVersion,
        });
      },
 ...
 * This defensive write keeps metadata valid for existing installations that may
 * have had metadata removed manually via browser tools.
 *
 * @returns {Promise<import('https://cdn.jsdelivr.net/npm/idb@8.0.3/+esm').IDBPDatabase>}
 */
