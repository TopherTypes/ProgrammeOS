...
let currentRaidTab = 'Risk';
let modalState = { type:null, index:null, tab:'overview', edit:false };
let crudState = { type:'Project', step:0, context:'global' };
...
function renderNav(){
  nav.innerHTML = navItems.map(item => `<button class="nav-item ${currentView===item.key?'active':''}" data-view="${item.key}"><span>${item.label}</span><span class="dot"></span></button>`).join('');
  nav.querySelectorAll('.nav-item').forEach(btn => btn.addEventListener('click', ()=>{ currentView = btn.dataset.view; render(); }));
}
function metrics(){
...
function renderRaid(){
  ...
  const tabs = ['Risk','Action','Issue','Decision'].map(tab=>`<button class="tab ${currentRaidTab===tab?'active':''}" data-raid-tab="${tab}">${tab}</button>`).join('');
  const filtered = data.raidGlobal.filter(r=>r.type===currentRaidTab);
  ...
  } else if (currentRaidTab==='Risk' || currentRaidTab==='Issue'){
...
function dashboardProjectPanel(index){
...
function dashboardPersonPanel(index){
...
function dashboardActionPanel(index){
...
function dashboardRaidPanel(index){
...
function openModal(type, index){
  modalState = { type, index, tab: type==='project'?'overview':'details', edit:false };
  renderModal();
  modalBackdrop.classList.add('open');
}
function renderModal(){
  const {type,index,tab,edit} = modalState;
  modalTabs.innerHTML = '';
...
function projectTabButton(key,label){
  return `<button class="seg-btn ${modalState.tab===key?'active':''}" data-project-tab="${key}">${label}</button>`;
}
...
function projectViewBody(index, tab){
  const p = data.projects[index];
  if (tab==='overview'){
...
function projectEditBody(index, tab){
  const p = data.projects[index];
  if (tab==='overview'){
    return `<div class="detail-grid">
      <div class="mini-item">
...
function personModalBody(index){
  const p = data.people[index];
...
function updateModalBody(index){
...
function decisionModalBody(index){
...
function actionModalBody(index){
  const a = data.actions[index];
  return `<div class="modal-grid">
...
function openCrud(type='Project', context='global'){
  crudState = { type, step:0, context };
  renderCrud();
  document.getElementById('crud-backdrop').classList.add('open');
}
function renderCrud(){
  const cfg = crudConfigs[crudState.type] || crudConfigs['Project'];
  document.getElementById('crud-title').textContent = `Create ${crudState.type}`;
  document.getElementById('crud-subtitle').textContent = `${crudState.context === 'global' ? 'Global create flow' : 'Contextual create flow'} · progressive disclosure`;
  document.getElementById('crud-note').textContent = cfg.note;
  document.getElementById('crud-steps').innerHTML = cfg.steps.map((s,i)=>`<div class="step ${i<crudState.step?'done':i===crudState.step?'active':''}">${i+1}. ${s}</div>`).join('');
  document.getElementById('crud-content').innerHTML = crudStepBody(crudState.type, crudState.step);
  document.getElementById('crud-prev').style.visibility = crudState.step === 0 ? 'hidden' : 'visible';
  document.getElementById('crud-next').textContent = crudState.step === cfg.steps.length-1 ? 'Create' : 'Next';
}
function crudStepBody(type, step){
  if (type==='Project'){
...
function wireInteractions(){
  document.querySelectorAll('[data-click]').forEach(el=>el.addEventListener('click',()=>{
    const value = el.dataset.click;
    const fromDashboard = el.dataset.source === 'dashboard';
    if (fromDashboard){
  ...
  document.querySelectorAll('[data-goto]').forEach(btn=>btn.addEventListener('click',()=>{ currentView = btn.dataset.goto; render(); }));
  document.querySelectorAll('[data-raid-tab]').forEach(btn=>btn.addEventListener('click',()=>{ currentRaidTab = btn.dataset.raidTab; renderRaid(); }));
  document.querySelectorAll('[data-project-tab]').forEach(btn=>btn.addEventListener('click',()=>{ modalState.tab = btn.dataset.projectTab; renderModal(); }));
  document.querySelectorAll('[data-create]').forEach(btn=>btn.addEventListener('click',()=> openCrud(btn.dataset.create, btn.dataset.context || 'global')));
  wireChecks();
}
...
document.getElementById('modal-edit-btn').addEventListener('click',()=>{
  modalState.edit = !modalState.edit;
  document.getElementById('modal-edit-btn').textContent = modalState.edit ? 'View mode' : 'Edit mode';
  renderModal();
});

document.getElementById('global-create').addEventListener('click',()=>openCrud('Project','global'));
document.getElementById('crud-close').addEventListener('click',()=>document.getElementById('crud-backdrop').classList.remove('open'));
document.getElementById('crud-cancel').addEventListener('click',()=>document.getElementById('crud-backdrop').classList.remove('open'));
document.getElementById('crud-prev').addEventListener('click',()=>{
  if (crudState.step>0){ crudState.step--; renderCrud(); }
});
document.getElementById('crud-next').addEventListener('click',()=>{
  const max = (crudConfigs[crudState.type] || crudConfigs['Project']).steps.length - 1;
  if (crudState.step < max){ crudState.step++; renderCrud(); }
  else { document.getElementById('crud-backdrop').classList.remove('open'); }
});
document.getElementById('crud-context').addEventListener('click',()=>{
  crudState.context = crudState.context === 'global' ? 'contextual' : 'global';
  renderCrud();
});
...
function render(){
  ...
  if (currentView==='meetings') return renderMeetings();
  if (currentView==='updates') return renderUpdates();
  if (currentView==='decisions') return renderDecisions();
  if (currentView==='actions') return renderActions();
  if (currentView==='raid') return renderRaid();
